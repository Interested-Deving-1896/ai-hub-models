...
from qai_hub_models.models.yolov4.model import YoloV4Tiny


class YoloV4TinyDetectionApp(YoloObjectDetectionApp):
    def check_image_size(self, pixel_values: torch.Tensor) -> None:
        ...
        of `YoloV4Tiny.STRIDE_MULTIPLE`.
        """
        if len(pixel_values.shape) != 4:
            raise ValueError("Pixel Values must be rank 4: [batch, channels, x, y]")
        if (
            pixel_values.shape[2] % YoloV4Tiny.STRIDE_MULTIPLE != 0
            or pixel_values.shape[3] % YoloV4Tiny.STRIDE_MULTIPLE != 0
        ):
            raise ValueError(
                f"Pixel values must have spatial dimensions (H & W) that are multiples of {YoloV4Tiny.STRIDE_MULTIPLE}."
            )
    ...
    def pre_nms_postprocess(
        ...
        *predictions
        ...
        if len(predictions) == 3:
            return detect_postprocess_split_input(*predictions)
