...
def test_find_matching_releases_first_only_vs_all(mock_find: MagicMock) -> None:
    ...
    assert [
        v for v, _ in find_matching_releases("m", versions=versions, first_only=True)
    ] == [Version("0.54.0")]
    assert [
        v for v, _ in find_matching_releases("m", versions=versions, first_only=False)
    ] == [Version("0.54.0"), Version("0.52.0")]
